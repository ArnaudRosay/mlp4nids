...
def main():
    ...
    input_path = "./cicids2017/traffic_types/"
    ...
    (df_train,
     ...
     df_test) = split_dataset(
        df_list, randomize=True, training_percentage=50,
    ...
    print("[INFO] {} - Writing Parquet files ...".format(now))
    logdir = "{}ids2017-run-{}/".format(output_path, now)
    try:
        os.mkdir(logdir)
    except OSError as err:
        print("Creation of directory {} failed:{}".format(logdir, err))
    df_train.to_parquet(logdir+"train_set.parquet")
    df_cv.to_parquet(logdir+"crossval_set.parquet")
    df_test.to_parquet(logdir+"test_set.parquet")
